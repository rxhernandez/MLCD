...
def load_CD_data():
    csv_path = CSN_path + "ML_CD_database_test1_test3_with_Pred_Color_woTT_csv.csv"
...
def load_CD_data():
    csv_path = CSN_path + "ML_CD_database_test1_test3_with_Pred_Color_woTT_only_csv.csv"
    return pd.read_csv(csv_path)
...
CD_test_exp1 = CD_test2[-16:]
CD_test_lit1 = CD_test2[:-16]
...
test_v = np.zeros((CD_test_exp1.shape[0], modelnum))
num_test_v = np.zeros((CD_test_exp1.shape[0]))
...
for i in range(modelnum):
    ...
    test = CD_test_exp1#CSN_prepared.loc[~CSN_prepared.index.isin(train.index), :]
    ...
    nall_f = norm(CD_test_exp1.drop(['Main Peak (in water)'], axis=1), train_f)
...
np.savetxt('Fig/test_set_trainMC_CDtestexp1_woTT_testXiao_0517.dat', train_v)
np.savetxt('Fig/test_set_train_numMC_CDtestexp1_woTT_testXiao_0517.dat', num_train_v)
np.savetxt('Fig/test_set_testMC_CDtestexp1_woTT_testXiao_0517.dat', test_v)
np.savetxt('Fig/test_set_test_numMC_CDtestexp1_woTT_testXiao_0517.dat', num_test_v)

#train_v = np.loadtxt('Figures/residuals_train.dat')
#num_train_v = np.loadtxt('Figures/residuals_train_num.dat')
test_v = np.loadtxt('Fig/test_set_testMC_CDtestexp1_woTT_testXiao_0517.dat')
num_test_v = np.loadtxt('Fig/test_set_test_numMC_CDtestexp1_woTT_testXiao_0517.dat')
...
test_err = np.where(np.isclose(test_v,0), 0, (test_v.T-CD_test_exp1['Main Peak (in water)'].values).T)
...
print('test', rc(avg_test_v, CD_test_exp1['Main Peak (in water)']))

print(stats.linregress(avg_test_v, CD_test_exp1['Main Peak (in water)']))
m, b = sp.stats.linregress(avg_test_v, CD_test_exp1['Main Peak (in water)'])[0:2]
...
plt.errorbar(avg_test_v, CD_test_exp1['Main Peak (in water)'], xerr=std_test_err, 
